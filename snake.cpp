...
snake::snake(int x , int y , int leng, int speed, Directions direction, int pic )
{
	...
	for (int i = 0; i < s_length; i++)
	{
		...
		{
		case UP: //向上，y坐标减一，x坐标不变
			tempX = s_headx;
			tempY = s_heady + i;
			break;
		case DOWN: //向下，y坐标减一，x坐标不变
			tempX = s_headx;
			tempY = s_heady - i;
			break;
		case LEFT: //向左，x坐标加一，y坐标不变
			tempX = s_headx + i;
			tempY = s_heady;
			break;
		case RIGHT: //向右，x坐标减一，y坐标不变
			tempX = s_headx - i;
			tempY = s_heady;
			break;
		}
