...
void game::run()//游戏运行
{
	...
	while (1) 
	{
		...
		if (checkFailed())//检测是否失败
		{
			...
			//break;
			int flag;
			flag = MessageBox(NULL, "是否再来一次？", "你输了！！！", MB_YESNO);//弹出对话框
			if (flag == 7)//选择否，结束程序
			{
				system("cls");//清屏
				break;
...
void game::drawGameInfo()//绘制游戏信息
{
	...
	cout << "游戏规则： 通过方向键控制贪吃蛇wasd移动;空格键暂停" << endl;
